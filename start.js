...
function script(data) {
    ...
    if (fs.existsSync('./user/hackerman.json')) {
        const MIo = require("./utils/main.js");
        let cookie = { appState: JSON.parse(fs.readFileSync("./user/hackerman.json", "utf8")) }
            log.comp("Starting ... ");
            try {
                MIo(cookie);
            } catch(e) {
        ...
        log.warning("Đang tạo file cookies ... ");
            ...
            login({email: data.user[0].email, password: data.user[0].password}, (err, bot) => {
                ...
                fs.writeFileSync('./user/hackerman.json', JSON.stringify(bot.getAppState()));
                script(data);
            });
    }
}
